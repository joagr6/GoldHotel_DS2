...
<head>
    ...
    <title>Nova Reserva - Gold Hotel</title>
...
<body class="bg-light">
    ...
    <div class="container py-5">
        <h1 class="fw-bold mb-4">Fazer Reserva</h1>
        ...
        <form method="POST" action="{{ route('reserva.store') }}" class="bg-white p-4 rounded shadow-sm">
            @csrf
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" name="quarto_id" required>
                        ...
                        @foreach ($quartos as $q)
                            <option value="{{ $q->id }}">{{ $q->tipoQuarto }} - R$ {{ number_format($q->valorDiaria, 2, ',', '.') }}</option>
                        @endforeach
                    </select>
                </div>
            @endif
            ...
            <div class="d-flex gap-2">
                ...
                <a href="{{ route('quarto.dashboard') }}" class="btn btn-secondary">Cancelar</a>
            </div>
        </form>
    </div>
